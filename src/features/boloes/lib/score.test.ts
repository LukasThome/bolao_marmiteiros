...
describe("calcularPontos", () => {
  describe("placar exato → 10 pontos", () => {
    it("vitória do mandante", () => {
      expect(calcularPontos({ homeScore: 2, awayScore: 1 }, { homeScore: 2, awayScore: 1 })).toBe(10);
    });
    it("empate", () => {
      expect(calcularPontos({ homeScore: 0, awayScore: 0 }, { homeScore: 0, awayScore: 0 })).toBe(10);
    });
    it("vitória do visitante", () => {
      expect(calcularPontos({ homeScore: 1, awayScore: 3 }, { homeScore: 1, awayScore: 3 })).toBe(10);
    });
    it("goleada", () => {
      expect(calcularPontos({ homeScore: 5, awayScore: 0 }, { homeScore: 5, awayScore: 0 })).toBe(10);
    });
  });

  describe("resultado certo, placar errado → 5 pontos", () => {
    it("acertou vitória do mandante", () => {
      expect(calcularPontos({ homeScore: 1, awayScore: 0 }, { homeScore: 3, awayScore: 1 })).toBe(5);
    });
    it("acertou empate", () => {
      expect(calcularPontos({ homeScore: 0, awayScore: 0 }, { homeScore: 2, awayScore: 2 })).toBe(5);
    });
    it("acertou vitória do visitante", () => {
      expect(calcularPontos({ homeScore: 0, awayScore: 2 }, { homeScore: 1, awayScore: 3 })).toBe(5);
    });
  });
